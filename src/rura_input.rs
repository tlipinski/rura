...
use crate::rura_input::ExecuteType::{
    Full, FullLive, UntilCurrent, UntilCurrentLive, UntilCurrentPrev,
};
use crate::rura_input::State::{
...
impl RuraInput {
    pub fn new(command: &str, cursor: usize) -> Result<RuraInput, ParseError> {
    ...
    pub fn command(&self, execute_type: &ExecuteType) -> Rura {
        match execute_type {
            Full | FullLive => Rura {
                steps: self.subcommands.clone(),
            },
...
#[cfg(test)]
mod tests {
    use crate::rura_input::ExecuteType::{Full, UntilCurrent, UntilCurrentPrev};
    use crate::rura_input::{RuraInput, split_command};

    #[test]
    fn commands() {
        let rura = RuraInput::new("a|b|c", 0).unwrap();
        assert_eq!(
            rura.command(&Full),
            Rura {
        ...
        assert_eq!(rura.command(&UntilCurrentPrev), Rura::empty());

        let rura = RuraInput::new("a|b|c", 1).unwrap();
        assert_eq!(
            rura.command(&Full),
            Rura {
                steps: vec!["a".into(), "b".into(), "c".into()],
        ...
        let rura = RuraInput::new("a|b|c", 2).unwrap();
        assert_eq!(
            rura.command(&Full),
            Rura {
                steps: vec!["a".into(), "b".into(), "c".into()],
        ...
        let rura = RuraInput::new("a|b|c", 3).unwrap();
        assert_eq!(
            rura.command(&Full),
            Rura {
                steps: vec!["a".into(), "b".into(), "c".into()],
        ...
        let rura = RuraInput::new("a|b|c", 4).unwrap();
        assert_eq!(
            rura.command(&Full),
            Rura {
                steps: vec!["a".into(), "b".into(), "c".into()],
    ...
    #[test]
    fn test_cursor_next() {
        let rura = RuraInput::new("aaa|bbbb|ccccc", 0).unwrap();
        assert_eq!(rura.cursor_next(true), Some(2));
        let rura = RuraInput::new("aaa|bbbb|ccccc", 1).unwrap();
        assert_eq!(rura.cursor_next(true), Some(2));

        let rura = RuraInput::new("aaa|bbbb|ccccc", 2).unwrap();
        assert_eq!(rura.cursor_next(true), Some(7));
        let rura = RuraInput::new("aaa|bbbb|ccccc", 3).unwrap();
        assert_eq!(rura.cursor_next(true), Some(7));
        let rura = RuraInput::new("aaa|bbbb|ccccc", 4).unwrap();
        assert_eq!(rura.cursor_next(true), Some(7));

        let rura = RuraInput::new("aaa|bbbb|ccccc", 7).unwrap();
        assert_eq!(rura.cursor_next(true), Some(14));

        let rura = RuraInput::new("aaa|bbbb|ccccc", 14).unwrap();
        assert_eq!(rura.cursor_next(true), Some(2));
    ...
    fn test_cursor_prev_no_cycle() {
        ...
        let rura = RuraInput::new("aaa|bbbb|ccccc", 13).unwrap();
        assert_eq!(rura.cursor_prev(), Some(7));
        let rura = RuraInput::new("aaa|bbbb|ccccc", 12).unwrap();
        assert_eq!(rura.cursor_prev(), Some(7));

        let rura = RuraInput::new("aaa|bbbb|ccccc", 8).unwrap();
        assert_eq!(rura.cursor_prev(), Some(7));
        let rura = RuraInput::new("aaa|bbbb|ccccc", 7).unwrap();
        assert_eq!(rura.cursor_prev(), Some(2));
        let rura = RuraInput::new("aaa|bbbb|ccccc", 6).unwrap();
        assert_eq!(rura.cursor_prev(), Some(2));
    ...
    fn test_insert_before() {
        ...
        let mut rura = RuraInput::new("aaa|bbbb", 5).unwrap();
    ...
    fn test_insert_after() {
        ...
        let mut rura = RuraInput::new("aaa|bbbb", 0).unwrap();
    ...
    #[test]
    fn test_ends() {
        let rura = RuraInput::new("", 0).unwrap();
    ...
    fn test_cursor_relative_position() {
        ...
        let rura = RuraInput::new("aaa|bbbb|ccccc", 9).unwrap();
        assert_eq!(rura.cursor_relative, 0);
    ...
    fn test_format_and_cursor_position() {
        ...
        let mut rura = RuraInput::new("aaa|bbbb|ccccc", 5).unwrap();
        let cursor = rura.format();
        assert_eq!(rura.to_string(), "aaa | bbbb | ccccc");
        assert_eq!(cursor, 7);
        ...
        let mut rura = RuraInput::new("aaa      |         bbbb       |      ccccc", 40).unwrap();
        let cursor = rura.format();
        assert_eq!(rura.to_string(), "aaa | bbbb | ccccc");
        assert_eq!(cursor, 16);
    }
